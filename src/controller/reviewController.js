...
const createReview = async function (req, res) {
  ...
  if (!data.reviewedAt || data.reviewedAt.trim().length == 0) {
    return res
      .status(400)
      .send({ status: false, msg: "ReviewedAt field is required" });
  }

  if (
    !/^\d{4}-(0[1-9]|1[0-2])-(0[1-9]|[12][0-9]|3[01])$/.test(data.reviewedAt)
  ) {
    return res.status(400).send({
      status: false,
      message: "ReviewedAt format should be in YYYY-MM-DD",
    });
  }
  ...
  if (!/^[0-5]{1,5}$/.test(data.rating)) {
    return res
      .status(400)
      .send({ status: false, message: "Rating should be in between 1 to 5" });
  }

  if (data.isDeleted === true)
    return res
      .status(400)
      .send({ status: false, msg: "This field value sholud be false" });
  ...
  const updateReview = await booksModel.findByIdAndUpdate(
    { _id: BookId },
    { $set: { reviews: countData } },
    { new: true }
  );
...
const deleteReview = async function (req, res) {
  try {
    ...
    if (deleteReviewData) {
        deleteReviewData1=await booksModel.findOneAndUpdate({ _id: book_id },{$inc:{ reviews: -1 }})
         }
