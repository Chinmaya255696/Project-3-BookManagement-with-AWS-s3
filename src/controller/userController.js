...
const createUser = async function (req, res) {
  try {
    ...
    if (!validator.isValidTitle(title)) {
      return res
        .status(400)
        .send({ status: false, msg: "Title should be among Mr, Mrs and Miss" });
    }
    ...
    if (!/^[0-9]\d{9}$/gi.test(phone)) {
      return res
        .status(400)
        .send({ status: false, message: `provide 10 digits Phone Number` });
    }
    const isPhoneNumberAlreadyUsed = await userModel.findOne({ phone });
    if (isPhoneNumberAlreadyUsed) {
      return res.status(400).send({
        status: false,
        message: `${phone} phone number is already registered`,
      });
    }
